...
class Jugador:
    ...
    """Metodo que define el movimiento del jugador en el trablero teniendo en cuenta a los enemigos para los algoritmos"""
    def move(self, dx, dy, casilla, bots):
        ...
        for i in range(len(casilla)):
            map.append([])
            for j in range(len(casilla[i])):
                map[i].append(casilla[i][j])
    ...
    """Metodo para setear la bomba en el trablero"""
    def plantar_bomba(self, map):
        b = Bomba(self.range, round(self.posX / 4), round(self.posY / 4), map, self)
        return b

    """Metodo para checkear la muerte del jugador"""
    ...
    """ Metodo para cargar y setear las animaciones del jugador"""
